...
async def _get_main_function_call_line(
    classname: str, first: bool = False, independent: bool = True
):
        ...
        if e and e.filenames and e.classname:
            ...
            calls.append(
                await _get_main_function_call_line(
                    classname=e.classname, first=(idx == 0), independent=e.independent
                )
            )
            ...
            for i, e_filename in enumerate(e.filenames):
                ...
                if i == 0:
                    import_line = await _get_import_line(
                        filename=e_filename, classname=e.classname
                    )
...
async def _create_requirements_file_in_workflow_dir(
    workflow_components: dict, dir_path: str
):
...
async def build_workflow(selected_elements: list) -> str:
    ...
    **selected_elements**: list of selected workflow elements' names
    ...
    await _copy_files_to_dir(
        files=workflow_components.get("files", []), dir_path=workflow_dir_path
    )
    await _create_main_file_in_workflow_dir(
        workflow_components=workflow_components, dir_path=workflow_dir_path
    )
    await _create_requirements_file_in_workflow_dir(
        workflow_components=workflow_components, dir_path=workflow_dir_path
    )
